...
import type {
  LateComingReportQuery,
  LateComingReportRecord,
} from "@/lib/api/types";
...
function asList(payload: unknown): LateComingReportRecord[] {
  if (Array.isArray(payload)) return payload as LateComingReportRecord[];
  const record = asRecord(payload);
  if (!record) return [];
  if (Array.isArray(record.data)) return record.data as LateComingReportRecord[];
  return [];
}
...
export function lateComingReportToRow(record: LateComingReportRecord): HrmsRow {
...
function withQuery(basePath: string, query?: LateComingReportQuery) {
  ...
  if (query?.min_late_minutes !== undefined && String(query.min_late_minutes) !== "") {
    params.set("min_late_minutes", String(query.min_late_minutes));
  }
...
export const lateComingReportService = {
  list: async (query?: LateComingReportQuery) => {
    const payload = await apiClient.get<unknown>(
      withQuery(API_ENDPOINTS.lateComingReport.list, query),
    );
    const rows = asList(payload).map(lateComingReportToRow);
